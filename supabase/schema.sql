...
create table if not exists public.events (
  ...
  constraint events_workspace_kind_values check (workspace_kind in ('share_time', 'travel_plan', 'habit_tracker')),
...
create index if not exists travel_stays_event_date_idx on public.travel_stays(event_id, check_in_date);
...
create trigger set_travel_stays_updated_at
before update on public.travel_stays
for each row execute function public.set_updated_at();

drop trigger if exists set_travel_journeys_updated_at on public.travel_journeys;
create trigger set_travel_journeys_updated_at
before update on public.travel_journeys
for each row execute function public.set_updated_at();
...
-- Remove confirmed periods from previous weeks when an ongoing Share time rolls over.
create or replace function public.rollover_event_time_plan(
  p_event_id uuid, p_week_start date
) returns jsonb
language plpgsql
security invoker
set search_path = ''
...
begin
  select * into v_event from public.events where id = p_event_id for update;
  if not found or v_event.workspace_kind <> 'share_time'
  ...
  return jsonb_build_object('finalPeriods', v_periods, 'finalTime',
    case when v_first.id is null then null else jsonb_build_object(
      'date', v_first.slot_date, 'startHour', v_first.start_hour,
      'finalizedAt', v_event.finalized_at
    ) end, 'finalNote',
    case when v_first.id is null then null else v_event.final_note end);
end;
$$;
revoke all on function public.rollover_event_time_plan(uuid, date) from public, anon, authenticated;
grant execute on function public.rollover_event_time_plan(uuid, date) to service_role;
